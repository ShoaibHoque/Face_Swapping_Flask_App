...
def index():
    ...
    if request.method == "POST":
        ...
        for triangle_index in indexes_triangles:
            ...
            warped_traingle = cv2.warpAffine(cropped_triangle, M, (w, h))
            warped_traingle = cv2.bitwise_and(warped_traingle, warped_traingle, mask=cropped_tr2_mask)

            # Reconstructing destination face
            img2_new_face_rect_area = img2_new_face[y:y+h, x:x+w]
            img2_new_face_rect_area_gray = cv2.cvtColor(img2_new_face_rect_area, cv2.COLOR_BGR2GRAY)
            _, mask_triangles_designed = cv2.threshold(img2_new_face_rect_area_gray, 1, 255, cv2.THRESH_BINARY_INV)
            warped_traingle = cv2.bitwise_and(warped_traingle, warped_traingle, mask=mask_triangles_designed)

            img2_new_face_rect_area = cv2.add(img2_new_face_rect_area, warped_traingle)
        ...
        resulting = Image.fromarray(seamlessclone, 'RGB')
        resulting.save(os.path.join(app.config['INITIAL_FILE_UPLOADS'], rname))
